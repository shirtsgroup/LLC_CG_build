...
import os
import mdtraj as md
import atom_props, transform, file_rw
import numpy as np
import sys
import subprocess
...
class ReadItp(object):
    ...
    def __init__(self, name):
        ...
        try:
            ...
            try:
                f = open('%s/../topologies/%s.itp' % (script_location, name), 'r')
            except FileNotFoundError:
...
class Residue(ReadItp):
    ...
    def __init__(self, name, connectivity=False):
        ...
        with open('%s/../topologies/ions.txt' % script_location) as f:
            ions = []
            for line in f:
                if line[0] != '#':
    ...
    def __init__(self, name):
        ...
        if name in ions:
            ...
            try:
                ...
                try:
                    t = md.load('%s/../topologies/%s.pdb' % (script_location, name), standard_names=False)  # check if the configuration is
...
class LC(ReadItp):
    ...
    def __init__(self, name):
        ...
        a = []
        with open('%s/../topologies/%s.gro' % (script_location, name)) as f:
            for line in f:
                a.append(line)

        self.t = md.load("%s/../topologies/%s.gro" % (script_location, name))
...
class Solute(Residue):
    ...
    def __init__(self, name):
        ...
        self.gro = []
        with open('%s/../topologies/%s.gro' % (script_location, name), 'r') as f:
            for line in f:
