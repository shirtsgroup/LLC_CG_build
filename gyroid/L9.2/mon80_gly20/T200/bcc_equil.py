...
import xlink, topology, restrain, file_rw
import warnings
import mdtraj as md
import os
import numpy as np
...
class EquilibrateBCC(topology.LC):
    ...
    def generate_mdps(self, T=300, em_steps=-1, time_step=0.01, length=1000, p_coupling='isotropic',
...
if __name__ == "__main__":
    ...
    if not args.continue_solvated and not args.continue_xlinked:
        ...
        mv = "mv solvated_nvt* solvated_npt* npt_equil* npt_* nvt_equil* em* intermediates"
        p = subprocess.Popen(mv, shell=True)  # don't split mv because shell=True
        p.wait()
    ...
    #clean
    mv = "mv solvated_nvt* solvated_npt* npt_equil* npt_* nvt_equil* em_* *.mdp *.txt intermediates"
    p = subprocess.Popen(mv, shell=True)  # don't split mv because shell=True
    p.wait()
